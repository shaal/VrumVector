...
try{
  ...
  const graphSaved=await page.evaluate(async()=>{
    ...
    return {model:before.model,updates:b.info().graphLearning.updates};
  });
  assert.ok(graphSaved.updates>0);
  await page.reload();await ready();
  assert.equal(await page.evaluate(async()=>(await import('/AI-Car-Racer/gnnReranker.js')).serialize().model),graphSaved.model);
